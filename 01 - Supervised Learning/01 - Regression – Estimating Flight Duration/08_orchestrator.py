...
"""
==============================================================================
FLIGHT TELEMETRY REGRESSION - PIPELINE ORCHESTRATOR
==============================================================================
...
5. Runs 05_plot_predicted_vs_actual.py (scatter plot visualization)
6. Runs 06_residual_analysis.py (residual diagnostics)
7. Runs 07_final_report.py (comprehensive final report)
...
class Config:
    ...
    REQUIRED_LIBRARIES = [
        ...
        'seaborn', 'scipy', 'pickle'
    ]

    # Expected outputs after each step
    # Format: step_index: [(file_path, is_critical), ...]
    # is_critical: True = must exist for pipeline success, False =
    # optional/informational
    EXPECTED_OUTPUTS: Dict[int, List[Tuple[str, bool]]] = {
        0: [  # 01_exploratory_analysis.py
            (str(IMAGES_DIR / "target_hist.png"), True),
            (str(IMAGES_DIR / "target_boxplot.png"), True),
            (str(IMAGES_DIR / "heatmap_correlations.png"), True),
            (str(IMAGES_DIR / "box_weather_vs_target.png"), False),
            (str(RESULTS_DIR / "target_statistics.csv"), True),
            (str(RESULTS_DIR / "eda_summary.md"), True),
        ],
        1: [  # 02_preprocessing.py
            (str(ARTIFACTS_DIR / "preprocessor.pkl"), True),
            (str(ARTIFACTS_DIR / "X_train.pkl"), True),
            (str(ARTIFACTS_DIR / "X_test.pkl"), True),
            (str(ARTIFACTS_DIR / "y_train.pkl"), True),
            (str(ARTIFACTS_DIR / "y_test.pkl"), True),
            (str(RESULTS_DIR / "feature_names_after_oh.csv"), True),
        ],
        2: [  # 03_train_models.py
            (str(ARTIFACTS_DIR / "model_linear_simple.pkl"), True),
            (str(ARTIFACTS_DIR / "model_linear_multiple.pkl"), True),
            (str(ARTIFACTS_DIR / "model_ridge.pkl"), True),
            (str(ARTIFACTS_DIR / "model_lasso.pkl"), True),
            (str(ARTIFACTS_DIR / "model_polynomial_deg2.pkl"), True),
            (str(ARTIFACTS_DIR / "poly_transformer_deg2.pkl"), True),
            (str(PREDICTIONS_DIR / "preds_linear_simple.csv"), True),
            (str(PREDICTIONS_DIR / "preds_linear_multiple.csv"), True),
            (str(PREDICTIONS_DIR / "preds_ridge.csv"), True),
            (str(PREDICTIONS_DIR / "preds_lasso.csv"), True),
            (str(PREDICTIONS_DIR / "preds_polynomial_deg2.csv"), True),
            (str(RESULTS_DIR / "training_times.csv"), True),
        ],
        3: [  # 04_evaluate_metrics.py
            (str(METRICS_FILE), True),
            (str(RESULTS_DIR / "model_comparison.md"), True),
            (str(ARTIFACTS_DIR / "best_model.txt"), True),
        ],
        4: [  # 05_plot_predicted_vs_actual.py
            (str(IMAGES_DIR / "predicted_vs_actual.png"), True),
            (str(PREDICTIONS_DIR / "predicted_vs_actual_data.csv"), False),
        ],
        5: [  # 06_residual_analysis.py
            (str(IMAGES_DIR / "residuals_hist.png"), True),
            (str(IMAGES_DIR / "residuals_vs_predictions.png"), True),
        ],
        6: [  # 07_final_report.py
            (str(FINAL_REPORT), True),
        ]
    }
...
def check_dependencies() -> bool:
    ...
    for lib in Config.REQUIRED_LIBRARIES:
        try:
            ...
            if lib not in ['pickle']:  # pickle is built-in
                missing.append(lib)
...
def verify_outputs(step_index: int) -> bool:
    ...
    for output, is_critical in Config.EXPECTED_OUTPUTS[step_index]:
        path = Path(output)
        if path.exists():
            if path.is_file():
                ...
                status = "✓" if is_critical else "✓"
                print_colored(f"  {status} {output}/", Fore.GREEN)
        else:
            if is_critical:
                print_colored(
                    f"  ✗ {output} - CRITICAL FILE MISSING!", Fore.RED)
                log(f"Critical output not found: {output}", "ERROR")
                missing_critical.append(output)
                all_critical_exist = False
            else:
                print_colored(
                    f"  ⚠️  {output} - optional (not found)",
                    Fore.YELLOW)
                log(f"Optional output not found: {output}", "WARNING")
                missing_optional.append(output)

    if missing_critical:
        print()
        print_colored(
            f"⚠️  {
                len(missing_critical)} critical file(s) missing!",
            Fore.RED,
            bright=True)
        print_colored(
            "     This may cause issues in subsequent steps.",
            Fore.YELLOW)

    if missing_optional:
        print_colored(
            f"     {
                len(missing_optional)} optional file(s) missing (OK).",
            Fore.CYAN)
...
def check_all_outputs() -> None:
    ...
    outputs_to_check = [
        ...
        (Config.RESULTS_DIR / "eda_summary.md", "EDA summary"),
        (Config.METRICS_FILE, "Metrics comparison"),
        (Config.ARTIFACTS_DIR / "preprocessor.pkl", "Preprocessor"),
        (Config.ARTIFACTS_DIR / "y_test.pkl", "Test set"),
        (Config.ARTIFACTS_DIR / "best_model.txt", "Best model info"),
        (Config.IMAGES_DIR / "predicted_vs_actual.png", "Predicted vs Actual plot"),
        (Config.IMAGES_DIR / "residuals_hist.png", "Residuals histogram"),
        (Config.IMAGES_DIR / "residuals_vs_predictions.png", "Residuals scatter"),
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Orchestrator for Flight Telemetry Regression ML Pipeline",
