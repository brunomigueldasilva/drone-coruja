...
"""
==============================================================================
FLIGHT INCIDENT PREDICTION - PIPELINE ORCHESTRATOR
==============================================================================
...
2. Runs 02_preprocessing.py (data preparation and encoding)
3. Runs 03_train_models.py (ensemble model training)
4. Runs 04_evaluate_metrics.py (performance evaluation)
5. Runs 05_roc_and_importance.py (ROC curves and feature importance)
6. Runs 06_partial_dependence.py (XAI with partial dependence plots)
7. Runs 07_final_report.py (comprehensive final report generation)
...
class Config:
    ...
    SCRIPTS = [
        ...
        "05_roc_and_importance.py",
        "06_partial_dependence.py",
        "07_final_report.py"
    ]
    ...
    SCRIPT_NAMES = [
        ...
        "Partial Dependence Analysis (XAI)",
    ...
    EXPECTED_OUTPUTS: Dict[int, List[str]] = {
        0: [  # 01_exploratory_analysis.py
            ...
            str(INPUT_DIR / "voos_pre_voo_clean.csv"),
            str(RESULTS_DIR / "descriptive_statistics.csv"),
            str(RESULTS_DIR / "correlation_with_target.csv"),
            str(RESULTS_DIR / "class_distribution.csv")
        ],
        1: [  # 02_preprocessing.py
            str(DATA_PROCESSED_DIR / "X_train_scaled.csv"),
            str(DATA_PROCESSED_DIR / "X_test_scaled.csv"),
            str(DATA_PROCESSED_DIR / "y_train.csv"),
            str(DATA_PROCESSED_DIR / "y_test.csv"),
            str(DATA_PROCESSED_DIR / "scaler.pkl"),
            str(DATA_PROCESSED_DIR / "feature_names.pkl")
        ],
        2: [  # 03_train_models.py
            str(ARTIFACTS_DIR / "decisiontree_model.pkl"),
            str(ARTIFACTS_DIR / "randomforest_model.pkl"),
            str(ARTIFACTS_DIR / "xgboost_model.pkl"),
            str(ARTIFACTS_DIR / "gradientboosting_model.pkl"),
            str(PREDICTIONS_DIR / "decisiontree_predictions.csv"),
            str(PREDICTIONS_DIR / "randomforest_predictions.csv"),
            str(PREDICTIONS_DIR / "xgboost_predictions.csv"),
            str(PREDICTIONS_DIR / "gradientboosting_predictions.csv")
        ],
        3: [  # 04_evaluate_metrics.py
            str(METRICS_FILE),
            str(OUTPUT_DIR / "metrics_comparison.md"),
            str(GRAPHICS_DIR / "confusion_matrices.png")
        ],
        4: [  # 05_roc_and_importance.py
            str(GRAPHICS_DIR / "roc_curves_comparison.png"),
            str(GRAPHICS_DIR / "feature_importance_rf.png"),
            str(GRAPHICS_DIR / "feature_importance_xgb.png"),
            str(GRAPHICS_DIR / "feature_importance_comparison.png")
        ],
        5: [  # 06_partial_dependence.py
            str(GRAPHICS_DIR / "partial_dependence_top3.png"),
            str(GRAPHICS_DIR / "partial_dependence_interaction.png")
        ],
...
def check_dependencies() -> bool:
    ...
    for lib in Config.REQUIRED_LIBRARIES:
        try:
            ...
            print_colored(f"  ✗ {lib}", Fore.RED)
            missing.append(lib)

    if missing:
        print()
        print_colored("⚠️  Missing dependencies!", Fore.YELLOW, bright=True)
...
def check_input_data() -> bool:
    ...
    input_path = Path(Config.INPUT_DATASET)
    ...
    if input_path.exists():
        ...
        log(f"Input dataset found: {Config.INPUT_DATASET}")
        return True
    else:
        print_colored(
            f"  ✗ Input dataset not found: {
                Config.INPUT_DATASET}",
            Fore.RED)
        print("\nPlease ensure the input file exists before running the pipeline.")
        log(
            f"Input dataset not found: {
                Config.INPUT_DATASET}",
            "ERROR")
...
def check_all_outputs() -> None:
    ...
    outputs_to_check = [
        ...
        (Config.INPUT_DIR / "voos_pre_voo_clean.csv", "Cleaned dataset", False),

        # Data processing outputs
        (Config.DATA_PROCESSED_DIR, "Processed data directory", True),
        (Config.DATA_PROCESSED_DIR / "X_train_scaled.csv",
         "Training features (scaled)", False),
        (Config.DATA_PROCESSED_DIR / "X_test_scaled.csv",
         "Test features (scaled)", False),
        (Config.DATA_PROCESSED_DIR / "y_train.csv", "Training labels", False),
        (Config.DATA_PROCESSED_DIR / "y_test.csv", "Test labels", False),
        (Config.DATA_PROCESSED_DIR / "scaler.pkl", "Scaler object", False),
        (Config.DATA_PROCESSED_DIR / "feature_names.pkl", "Feature names", False),

        # Model artifacts
        (Config.ARTIFACTS_DIR, "Model artifacts directory", True),
        (Config.ARTIFACTS_DIR / "decisiontree_model.pkl",
         "Decision Tree model", False),
        (Config.ARTIFACTS_DIR / "randomforest_model.pkl",
         "Random Forest model", False),
        (Config.ARTIFACTS_DIR / "xgboost_model.pkl", "XGBoost model", False),
        (Config.ARTIFACTS_DIR / "gradientboosting_model.pkl",
         "Gradient Boosting model", False),

        # Predictions
        (Config.PREDICTIONS_DIR, "Predictions directory", True),
        (Config.PREDICTIONS_DIR /
         "decisiontree_predictions.csv", "DT predictions", False),
        (Config.PREDICTIONS_DIR /
         "randomforest_predictions.csv", "RF predictions", False),
        (Config.PREDICTIONS_DIR / "xgboost_predictions.csv", "XGB predictions", False),
        (Config.PREDICTIONS_DIR /
         "gradientboosting_predictions.csv", "GB predictions", False),

        # Results and metrics
        (Config.RESULTS_DIR, "Results directory", True),
        (Config.METRICS_FILE, "Metrics comparison CSV", False),
        (Config.OUTPUT_DIR / "metrics_comparison.md", "Metrics comparison MD", False),

        # Graphics
        (Config.GRAPHICS_DIR, "Graphics directory", True),
        (Config.GRAPHICS_DIR / "confusion_matrices.png", "Confusion matrices", False),
        (Config.GRAPHICS_DIR / "roc_curves_comparison.png", "ROC curves", False),
        (Config.GRAPHICS_DIR / "feature_importance_rf.png",
         "RF feature importance", False),
        (Config.GRAPHICS_DIR / "feature_importance_xgb.png",
         "XGB feature importance", False),
        (Config.GRAPHICS_DIR / "feature_importance_comparison.png",
         "Feature importance comparison", False),
        (Config.GRAPHICS_DIR / "partial_dependence_top3.png",
         "Partial dependence (1D)", False),
        (Config.GRAPHICS_DIR / "partial_dependence_interaction.png",
         "Partial dependence (2D)", False),
