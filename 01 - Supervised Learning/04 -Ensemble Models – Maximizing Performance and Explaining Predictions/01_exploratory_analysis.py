...
def save_cleaned_dataset(
    ...
    # Save to CSV
    output_path = Config.INPUT_DIR / 'voos_pre_voo_clean.csv'
...
def plot_target_distribution(
    ...
    """
    Create target distribution plots (absolute and percentage).

    Args:
        df: Input DataFrame

    Returns:
        Tuple of (value counts, labels, percentages)
    """
    print_section("1. TARGET DISTRIBUTION")

    target = df[Config.TARGET_COLUMN]
    counts = target.value_counts().sort_index()
    labels = ['No Incident', 'With Incident']
    percentages = (counts / len(df) * 100).tolist()

    print(f"Class 0 (No Incident): {counts[0]} ({percentages[0]:.1f}%)")
    print(f"Class 1 (With Incident): {counts[1]} ({percentages[1]:.1f}%)")
    print(f"Imbalance ratio: {counts[0] / counts[1]:.2f}:1")

    # Create figure with 2 subplots
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 5))

    # 1. Absolute distribution (bar plot)
    x = np.arange(len(labels))
    bars = ax1.bar(x, counts.values, color=Config.COLORS_TARGET,
                   edgecolor='black', linewidth=1.5)
    ax1.set_xticks(x)
    ax1.set_xticklabels(labels, fontsize=11)
    ax1.set_ylabel('Count', fontsize=12)
    ax1.set_title('Absolute Distribution', fontsize=13, fontweight='bold')
    ...
    colors_pie = Config.COLORS_TARGET
...
def plot_numeric_boxplots(
    ...
    """
    Create boxplots comparing numeric features by incident status.

    Args:
        df: Input DataFrame
        numeric_features: List of numeric feature column names
    """
    print_section("2. NUMERIC BOXPLOTS BY INCIDENT STATUS")
    ...
    for idx, col in enumerate(numeric_features):
        ...
        data_no = df[df[Config.TARGET_COLUMN] == 0][col]
        data_yes = df[df[Config.TARGET_COLUMN] == 1][col]
        ...
        ax.set_xlabel('Incident', fontsize=11)
...
def plot_numeric_histograms(
        ...
        numeric_features: List[str]) -> None:
    """
    Create histograms of numeric features separated by incident status.

    Args:
        df: Input DataFrame
        numeric_features: List of numeric feature column names
    """
    print_section("3. NUMERIC HISTOGRAMS BY INCIDENT STATUS")
    ...
    for idx, col in enumerate(numeric_features):
        ...
        title = col.replace('_', ' ').title()
        ax.set_title(title, fontsize=12, fontweight='bold')
        ax.legend(title='Incident', fontsize=9)
        ax.grid(axis='y', alpha=0.3)
...
def plot_categorical_analysis(df: pd.DataFrame) -> None:
    ...
    Args:
        df: Input DataFrame (original, not cleaned)
    """
    print_section("4. CATEGORICAL FEATURE ANALYSIS")

    fig = plt.figure(figsize=(14, 10))
    gs = fig.add_gridspec(2, 2, hspace=0.3, wspace=0.3)

    # Top row: Previsao Turbulencia
    ax1 = fig.add_subplot(gs[0, 0])
    ax2 = fig.add_subplot(gs[0, 1])

    # Distribution
    turb_counts = df['previsao_turbulencia'].value_counts()
    turb_order = ['Baixa', 'Média', 'Alta']
    turb_counts = turb_counts.reindex(turb_order)

    ax1.bar(range(len(turb_counts)), turb_counts.values,
            color='#FFB6C1', edgecolor='black', linewidth=1.5)
    ax1.set_xticks(range(len(turb_counts)))
    ax1.set_xticklabels(turb_order, fontsize=10)
    ax1.set_ylabel('Count', fontsize=11)
    ax1.set_title('Distribution - Previsao Turbulencia',
                  fontsize=12, fontweight='bold')
    ax1.grid(axis='y', alpha=0.3)

    # Incident rate
    turb_incident = df.groupby('previsao_turbulencia')[
        Config.TARGET_COLUMN].mean() * 100
    turb_incident = turb_incident.reindex(turb_order)

    ax2.bar(range(len(turb_incident)), turb_incident.values,
            color='#FF6B6B', edgecolor='black', linewidth=1.5)
    ax2.set_xticks(range(len(turb_incident)))
    ax2.set_xticklabels(turb_order, fontsize=10)
    ax2.set_xlabel('Previsao Turbulencia', fontsize=11)
    ax2.set_ylabel('% Incidents', fontsize=11)
    ax2.set_title('Incident Rate - Previsao Turbulencia',
                  fontsize=12, fontweight='bold')
    ax2.grid(axis='y', alpha=0.3)

    # Bottom row: Tipo Missao
    ax3 = fig.add_subplot(gs[1, 0])
    ax4 = fig.add_subplot(gs[1, 1])

    # Distribution
    missao_counts = df['tipo_missao'].value_counts()
    missao_order = ['Transporte', 'Carga', 'Vigilância']
    missao_counts = missao_counts.reindex(missao_order)

    ax3.bar(range(len(missao_counts)), missao_counts.values,
            color='#87CEEB', edgecolor='black', linewidth=1.5)
    ax3.set_xticks(range(len(missao_counts)))
    ax3.set_xticklabels(missao_order, fontsize=10)
    ax3.set_ylabel('Count', fontsize=11)
    ax3.set_title('Distribution - Tipo Missao',
                  fontsize=12, fontweight='bold')
    ax3.grid(axis='y', alpha=0.3)

    # Incident rate
    missao_incident = df.groupby('tipo_missao')[
        Config.TARGET_COLUMN].mean() * 100
    missao_incident = missao_incident.reindex(missao_order)

    ax4.bar(range(len(missao_incident)), missao_incident.values,
            color='#FF6B6B', edgecolor='black', linewidth=1.5)
    ax4.set_xticks(range(len(missao_incident)))
    ax4.set_xticklabels(missao_order, fontsize=10)
    ax4.set_xlabel('Tipo Missao', fontsize=11)
    ax4.set_ylabel('% Incidents', fontsize=11)
    ax4.set_title('Incident Rate - Tipo Missao',
                  fontsize=12, fontweight='bold')
